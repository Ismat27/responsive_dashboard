...
export const SideLink = ({ to, Img, text, sideExpand }: SideLinkProps) => {
  return (
    <li>
      <Link
        to={to}
        className="p-4 block hover:bg-gray-200 hover:bg-opacity-30 hover:border-l-4 hover:border-white transition-all ease-linear duration-300"
      >
        <span className="flex items-center flex-row gap-4 text-[15px] text-white capitalize font-medium">
          <ImageOrIcon Icon={Img} alt={text} />
          <span
            className={`overflow-hidden text-[15px] capitalize font-medium ${
              sideExpand ? "w-[auto]" : "w-0"
            }`}
          >
            {text}
          </span>
...
const Sidebar = ({ expand, setExpand, sideTranslate }: SideBarProps) => {
  return (
    <aside
      className={`bg-black-100 fixed top-[62px] left-0 z-10 h-[100vh] overflow-hidden overflow-y-auto py-4 transition-all ease-linear duration-300 ${
        expand ? "w-[255px]" : "w-[88px]"
      } ${
        sideTranslate ? "-translate-x-[255px]" : "translate-x-0"
